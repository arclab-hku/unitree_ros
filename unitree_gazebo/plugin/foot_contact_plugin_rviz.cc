...
namespace gazebo
{
    class UnitreeFootContactPluginRviz : public SensorPlugin
    {
        public:
        UnitreeFootContactPluginRviz() : SensorPlugin(){}
        ~UnitreeFootContactPluginRviz(){}
        ...
        void Load(sensors::SensorPtr _sensor, sdf::ElementPtr _sdf)
        {
            ...
            if (!this->parentSensor){
                gzerr << "UnitreeFootContactPluginRviz requires a ContactSensor.\n";
                return;
            }

            if (!_sdf->HasElement("linkName"))
            {
                ROS_FATAL_NAMED("FT", "foot cantact plugin missing <linkName>, cannot proceed");
                return;
            }
            else
            ...
            this->update_connection = this->parentSensor->ConnectUpdated(std::bind(&UnitreeFootContactPluginRviz::OnUpdate, this));
            this->parentSensor->SetActive(true); // Make sure the parent sensor is active.
            count = 0;
            Fx = 0;
            Fy = 0;
            Fz = 0;
            ROS_INFO("Load %s Rviz plugin.", _sensor->Name().c_str());
        }
        ...
        void OnUpdate()
        {
            ...
            force.header.frame_id = this->link_name_;
    ...
    GZ_REGISTER_SENSOR_PLUGIN(UnitreeFootContactPluginRviz)
}
