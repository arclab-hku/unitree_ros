...
void GazeboRosP3DTF::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  this->callback_queue_thread_ = boost::thread(
    boost::bind(&GazeboRosP3DTF::P3DQueueThread, this));

  // New Mechanism for Updating every World Cycle
  // Listen to the update event. This event is broadcast every
  // simulation iteration.
  this->update_connection_ = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&GazeboRosP3DTF::UpdateChild, this));
}
...
{
#ifdef ENABLE_PROFILER
  IGN_PROFILE("GazeboRosP3DTF::UpdateChild");
#endif
  if (!this->link_)
    return;
...
double GazeboRosP3DTF::GaussianKernel(double mu, double sigma)
{
...
void GazeboRosP3DTF::P3DQueueThread()
{
